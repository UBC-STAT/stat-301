...
test_2.0 <- function() {
    ...
    check_DF(US_county_sample,
            colnames(US_county_sample),
            "069c0949cc402f30063b90e9a31f87e1",
             c("avgAnnCount",      "avgDeathsPerYear", "TARGET_deathRate", "incidenceRate"),   
             rep(1,4),
             c("1166863e691aa0fde4773f3c725288c0",
               "e98808ca0bc39332b7304ba034eaac26",
               "25a82015ccad883a3ae4ab0939dcfc07",
               "5828643df541b81d2db67089dbb0429f"))
}
...
test_2.1 <- function() {
    check_DF(US_county_sample,
            c("replicate","avgAnnCount", "avgDeathsPerYear", "incidenceRate", "medIncome", "povertyPercent",
    "studyPerCap", "MedianAge", "PctPrivateCoverage", "PctPublicCoverage", "TARGET_deathRate"),
            "069c0949cc402f30063b90e9a31f87e1",
             c("avgAnnCount",      "avgDeathsPerYear", "TARGET_deathRate", "incidenceRate"),   
             rep(1,4),
             c("1166863e691aa0fde4773f3c725288c0",
...
test_2.2 <- function() {
    check_DF_overflow(fwd_summary_df,
            colnames(fwd_summary_df),
            "7c7124efff5c7039a1b1e7cba65c5379",
            colnames(fwd_summary_df),
            rep(1e4,4),
            c("3226d48d9ed2703d2b6cd561542639ff",
              "e234037d3a78124a8f8878bccf94c6e3",
              "f40d165f735a454fb0f10b8478f0a39f",
              "2d885784d741f5f1265c17221ca93b7d"))
    check_DF_overflow(back_summary_df,
            colnames(back_summary_df),
            "7c7124efff5c7039a1b1e7cba65c5379",
            colnames(back_summary_df),
            rep(1e4,4),
            c("3226d48d9ed2703d2b6cd561542639ff",
...
test_2.3 <- function() {
    check_MC(answer2.3, LETTERS[1:9], "91605cbd2f619446c431e683a84dfcd5")
}
...
test_2.4 <- function() {
    check_MC(answer2.4, LETTERS[1:9], "f6e1d4fc5f1beabd8eab925b3a572df7")
}
...
test_2.5 <- function() {
    check_DF(US_county_sample_candidate_models,
            c("n_input_variables", "C_p", "Method"),
            "06cd248dd1409b804444bd9ad5533d1d",
             c("n_input_variables", "C_p"),              
             c(1, 10000),
             c("57d435173cd01ba1272e9bbc2e1d0296",
               "8a9e8b32b79e34cc871ba1f20249af0f"))
}
...
test_2.7 <- function() {
      test_that(paste('Did not assign answer to an object called ', "fwd_data"), {
    expect_true(exists("fwd_data"))
  })
    
      test_that(paste('Did not assign answer to an object called ', "fwd_xlab"), {
    expect_true(exists("fwd_xlab"))
  })
    
      test_that(paste('Did not assign answer to an object called ', "fwd_ylab"), {
    expect_true(exists("fwd_ylab"))
  })
    
          test_that(paste('Did not assign answer to an object called ', "back_data"), {
    expect_true(exists("back_data"))
  })
    
      test_that(paste('Did not assign answer to an object called ', "back_xlab"), {
    expect_true(exists("back_xlab"))
  })
    
      test_that(paste('Did not assign answer to an object called ', "back_ylab"), {
    expect_true(exists("back_ylab"))
  })
    
    
    #===========
    
          test_that("fwd_data must be numeric", {
    expect_true(is.numeric(fwd_data))
  })
    
      test_that("fwd_xlab must be a string", {
    expect_true( is.character(fwd_xlab) & length(fwd_xlab) == 1)
  })
    
      test_that("fwd_ylab must be a string", {
    expect_true( is.character(fwd_ylab) & length(fwd_ylab) == 1)
  })
    
          test_that("back_data must be numeric", {
    expect_true(is.numeric(back_data))
  })
    
      test_that("back_xlab must be a string", {
    expect_true( is.character(back_xlab) & length(back_xlab) == 1)
  })
    
      test_that("back_ylab must be a string", {
    expect_true( is.character(back_ylab) & length(back_ylab) == 1)
  })
          ...
          #===========================
                    ...
                    test_that("fwd_data is incorrect", {
    expect_equal(digest(as.integer(sum(fwd_data))), "81dd0440e803b4824de9b950c118d91e")
  })
          
                    test_that("back_data is incorrect", {
    expect_equal(digest(as.integer(sum(fwd_data))), "81dd0440e803b4824de9b950c118d91e")
  })
          
          
    print("Success! Have a great day :)")
}
