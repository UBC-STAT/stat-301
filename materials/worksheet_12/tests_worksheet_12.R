...
test_1.0 <- function() {
  test_that('Did not assign answer to an object called "Default"', {
    expect_true(exists("Default"))
  })

  test_that("Solution should be a data frame", {
    expect_true("data.frame" %in% class(Default))
  })

  expected_colnames <- c("default", "student", "balance", "income")
  given_colnames <- colnames(Default)
  ...
  test_that("Data frame does not contain the correct number of rows", {
    expect_equal(digest(as.integer(nrow(Default))), "f1a30baa3072c1aad822c059f35c6841")
  })

  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(Default$balance))), "fe77e9bf661be6a426411dc60fe6b6cd")
  })
...
test_1.11 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(Default_binary_log_model_results$exp.estimate) * 10e6)), "7a4bb9d19684e603bffc6f1961a7eda1")
    expect_equal(digest(as.integer(sum(Default_binary_log_model_results$exp.conf.low) * 10e6)), "55679ab470e07149e5a38ce2213a15df")
    expect_equal(digest(as.integer(sum(Default_binary_log_model_results$exp.conf.high) * 10e6)), "bdccf6329a8314620c8305cc31634021")
  })
