...
# Question 2.0
test_2.0 <- function() {
    check_DF(US_cancer_data_head,
            c("TARGET_deathRate","povertyPercent" ,"PctPrivateCoverage"),
            "11946e7a3ed5e1776e81c0f0ecd383d0",
             c("TARGET_deathRate","povertyPercent" ,"PctPrivateCoverage"),
             c(10,10,10),
             c("fb5cdcd7c04540019b114fe68155e12c",
               "931952b6c4f91994416dca5c89da87de",
               "be5f9bfb244bd28afe5bc95e6f416b93")
            )
}
...
test_2.1 <- function() {
  ...
  test_that("Solution is incorrect", {
    expect_equal(digest(as.integer(answer_as_numeric)), "906679c1f1b0a7c8e05a254c9e04f3d2")
  })
...
test_2.2 <- function() {
    check_DF(US_cancer_data_stats, 
            c("key", "mean", "sd", "max", "min"),
             '11946e7a3ed5e1776e81c0f0ecd383d0',
             c("mean", "sd", "max", "min"),
             c(1e4,1e4,10,10),
             c("14201caf836bcceb0052689827ff656f",
               "338015807924035f0e74356e5c12f3bd",
               "8b67f4e96a2fcb7536ea2fbd9a0a5c8f",
               "463049b9a307ba66303cb2871e724126"))
...
test_2.3.0 <- function() {
  test_that('Did not assign answer to an object called "deathRate_hist"', {
    expect_true(exists("deathRate_hist"))
  })

  test_that("Solution should be a ggplot object", {
    expect_true(is.ggplot(deathRate_hist))
  })

  properties <- c(deathRate_hist$layers[[1]]$mapping, deathRate_hist$mapping)

  test_that("Plot should have TARGET_deathRate on the x-axis", {
    expect_true("TARGET_deathRate" == rlang::get_expr(properties$x))
  })

  test_that("Plot does not have the correct layers", {
    expect_true("GeomBar" %in% class(deathRate_hist$layers[[1]]$geom))

    # Remove if not needed:
    # expect_true("GeomVline" %in% class(deathRate_hist$layers[[2]]$geom))
  })

  test_that("Plot does not have the correct bin width", {
  expect_equal(
    digest(as.integer(mget("stat_params", deathRate_hist$layers[[1]])[["stat_params"]][["binwidth"]])),
    "3e2e4a08c44d0224de5b7e668c75ace3"
    )
  })

  test_that("Plot does not use the correct data", {
    expect_equal(digest(nrow(deathRate_hist$data)), "906679c1f1b0a7c8e05a254c9e04f3d2")
    expect_equal(digest(round(sum(deathRate_hist$data$TARGET_deathRate))), "bb0010e5a013c833cf47f2807c15ef85")

    # If X_AXIS_VAR is not known:
    # expect_equal(digest(round(sum(pull(deathRate_hist$data, rlang::get_expr(properties$x))))), "HASH_HERE")
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(deathRate_hist$labels$x == toString(rlang::get_expr(properties$x)))
  })
...
test_2.3.1 <- function() {
  test_that('Did not assign answer to an object called "povertyPercent_hist"', {
    expect_true(exists("povertyPercent_hist"))
  })

  test_that("Solution should be a ggplot object", {
    expect_true(is.ggplot(povertyPercent_hist))
  })

  properties <- c(deathRate_hist$layers[[1]]$mapping, povertyPercent_hist$mapping)
  ...
  test_that("Plot does not have the correct layers", {
    expect_true("GeomBar" %in% class(povertyPercent_hist$layers[[1]]$geom))
  ...
  test_that("Plot does not have the correct bin width", {
  expect_equal(
    digest(as.integer(mget("stat_params", povertyPercent_hist$layers[[1]])[["stat_params"]][["binwidth"]])),
    "3e2e4a08c44d0224de5b7e668c75ace3"
    )
  })

  test_that("Plot does not use the correct data", {
    expect_equal(digest(nrow(povertyPercent_hist$data)), "906679c1f1b0a7c8e05a254c9e04f3d2")
    expect_equal(digest(round(sum(povertyPercent_hist$data$povertyPercent))), "4a4d5dd218f223678e55d0fbb2fe0dd0")

    # If X_AXIS_VAR is not known:
    # expect_equal(digest(round(sum(pull(povertyPercent_hist$data, rlang::get_expr(properties$x))))), "HASH_HERE")
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(povertyPercent_hist$labels$x == toString(rlang::get_expr(properties$x)))
  })
...
test_2.3.2 <- function() {
  test_that('Did not assign answer to an object called "PctPrivateCoverage_hist"', {
    expect_true(exists("PctPrivateCoverage_hist"))
  })

  test_that("Solution should be a ggplot object", {
    expect_true(is.ggplot(PctPrivateCoverage_hist))
  })

  properties <- c(PctPrivateCoverage_hist$layers[[1]]$mapping, PctPrivateCoverage_hist$mapping)

  test_that("Plot should have PctPrivateCoverage on the x-axis", {
    expect_true("PctPrivateCoverage" == rlang::get_expr(properties$x))
  })

  test_that("Plot does not have the correct layers", {
    expect_true("GeomBar" %in% class(PctPrivateCoverage_hist$layers[[1]]$geom))
  ...
  test_that("Plot does not have the correct bin width", {
  expect_equal(
    digest(as.integer(mget("stat_params", PctPrivateCoverage_hist$layers[[1]])[["stat_params"]][["binwidth"]])),
    "3e2e4a08c44d0224de5b7e668c75ace3"
    )
  })
  ...
  test_that("Plot does not use the correct data", {
    expect_equal(digest(nrow(povertyPercent_hist$data)), "906679c1f1b0a7c8e05a254c9e04f3d2")
    expect_equal(digest(round(sum(PctPrivateCoverage_hist$data$PctPrivateCoverage))), "04123aea818c48abc11a49bc5d938dd0")

    # If X_AXIS_VAR is not known:
    # expect_equal(digest(round(sum(pull(PctPrivateCoverage_hist$data, rlang::get_expr(properties$x))))), "HASH_HERE")
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(PctPrivateCoverage_hist$labels$x == toString(rlang::get_expr(properties$x)))
  })
...
test_2.4 <- function() {
    check_MC(answer2.4, letters[1:3], "127a2ec00989b9f7faf671ed470be7f8")
}
...
test_3.0 <- function() {
    check_MC(answer3.0.0, c("TARGET_deathRate", "povertyPercent", "PCTPrivateCoverage"), "4188171f603a73990c4b0fc767fbb43f")
    check_MC(answer3.0.1, c("TARGET_deathRate", "povertyPercent", "PCTPrivateCoverage"), "875df41e6dd76148be710418bd7b5b0b")
}
#distinct
...
# Question 3.2
test_3.2 <- function() {
    check_MC(answer3.2, letters[1:2], 'ddf100612805359cd81fdc5ce3b9fbba')
}
...
# Question 4.3
test_4.3 <- function() {
    check_MC(answer4.3, letters[1:3], "6e7a8c1c098e8817e3df3fd1b21149d1")
}
...
# Question 4.5
test_4.5 <- function() {
    check_MC(answer4.5.0, letters[1:2], 'ddf100612805359cd81fdc5ce3b9fbba')
    check_MC(answer4.5.1, letters[1:2], '127a2ec00989b9f7faf671ed470be7f8')
}

# Question 5.0
test_5.0 <- function() {
    check_MC(answer5.0, letters[1:4], "6e7a8c1c098e8817e3df3fd1b21149d1")
}

# Question 5.1
test_5.1 <- function() {
    check_MC(answer5.1, letters[1:4], 'd110f00cfb1b248e835137025804a23b')
}

# Question 5.2
test_5.2 <- function() {
    check_MC(answer5.2, letters[1:4], '127a2ec00989b9f7faf671ed470be7f8')
}

# Question 5.3
test_5.3 <- function() {
    check_MC(answer5.3, letters[1:3], 'ddf100612805359cd81fdc5ce3b9fbba')
}
...
test_6.2 <- function() {
  ...
  test_that("Plot does not use the correct data", {
    ...
    expect_equal(digest(round(sum(slope_sampling_dist_500$data$boot_slope))), "a8ff2d667e1745ee0a5a7e17006a78fc")
