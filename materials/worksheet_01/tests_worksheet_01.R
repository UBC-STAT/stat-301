...
test_1.0 <- function() {
  check_MC(answer1.0.0, c("TARGET_deathRate", "povertyPercent", "PCTPrivateCoverage"), "4188171f603a73990c4b0fc767fbb43f")
  check_MC(answer1.0.1, c("TARGET_deathRate", "povertyPercent", "PCTPrivateCoverage"), "875df41e6dd76148be710418bd7b5b0b")
}
#distinct
...
# Question 1.2
test_1.2 <- function() {
    check_MC(answer1.2, letters[1:2], 'ddf100612805359cd81fdc5ce3b9fbba')
}

# Question 2.0
test_2.0 <- function() {
  check_MC(answer2.0, letters[1:2], 'ddf100612805359cd81fdc5ce3b9fbba')
}

# Question 2.1
test_2.1 <- function() {
  check_MC(answer2.1, letters[1:2], 'ddf100612805359cd81fdc5ce3b9fbba')
}
...
# Question 3.2
test_3.2 <- function() {
    check_MC(answer3.2, letters[1:3], "6e7a8c1c098e8817e3df3fd1b21149d1")
}
...
# Question 3.4
test_3.4 <- function() {
    check_MC(answer3.4.0, letters[1:2], 'ddf100612805359cd81fdc5ce3b9fbba')
    check_MC(answer3.4.1, letters[1:2], '127a2ec00989b9f7faf671ed470be7f8')
}
...
# Question 5.1
test_5.1 <- function() {
    check_MC(answer5.1, letters[1:4], "6e7a8c1c098e8817e3df3fd1b21149d1")
}

# Question 5.2
test_5.2 <- function() {
    check_MC(answer5.2, letters[1:4], 'd110f00cfb1b248e835137025804a23b')
}

# Question 5.3
test_5.3 <- function() {
    check_MC(answer5.3, letters[1:4], '127a2ec00989b9f7faf671ed470be7f8')
}

# Question 5.4
test_5.4 <- function() {
    check_MC(answer5.4, letters[1:3], 'ddf100612805359cd81fdc5ce3b9fbba')
}
...
#df
test_7.1.1 <- function() {
    check_DF(bootstrap_slope_infer,
            c("replicate", "stat"  ),
            "b6a6227038bf9be67533a45a6511cc7e",
             c("stat"),    
             c(10000),
             c("7b44c4fdbb2a98a84f489942a6692e61"))
}
...
test_7.2.1 <- function() {
  ...
  test_that("Plot does not use the correct data", {
    ...
    expect_equal(digest(round(sum(slope_sampling_dist_500$data$stat))), "dbc54d0980f93fffcb8e536628667f3f")
...
test_7.2.2 <- function() {
  test_that('Did not assign answer to an object called "slope_sampling_dist_1500"', {
    expect_true(exists("slope_sampling_dist_1500"))
  })

  test_that("Solution should be a ggplot object", {
    expect_true(is.ggplot(slope_sampling_dist_1500))
  })

  properties <- c(slope_sampling_dist_1500$layers[[1]]$mapping, slope_sampling_dist_1500$mapping)
  ...
  test_that("Plot does not have the correct layers", {
    expect_true("GeomBar" %in% class(slope_sampling_dist_1500$layers[[1]]$geom))

    # Remove if not needed:
    # expect_true("GeomVline" %in% class(slope_sampling_dist_1500$layers[[2]]$geom))
  })

  test_that("Plot does not have the correct bin width", {
  expect_equal(
    digest(as.integer(mget("stat_params", slope_sampling_dist_1500$layers[[1]])[["stat_params"]][["binwidth"]])),
    "3e2e4a08c44d0224de5b7e668c75ace3"
    )
  })

  test_that("Plot does not use the correct data", {
    expect_equal(digest(nrow(slope_sampling_dist_1500$data)), "b6a6227038bf9be67533a45a6511cc7e")
    expect_equal(digest(round(sum(slope_sampling_dist_1500$data$stat))), "815a99e2cd5739af9181ab39ac235c91")

    # If boot_slope is not known:
    # expect_equal(digest(round(sum(pull(slope_sampling_dist_1500$data, rlang::get_expr(properties$x))))), "HASH_HERE")
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(slope_sampling_dist_1500$labels$x == toString(rlang::get_expr(properties$x)))
  })

  test_that("Plot should have a title", {
    expect_true("title" %in% names(slope_sampling_dist_1500$labels))
  })
...
# Question 7.2.3
test_7.2.3 <- function() {
    check_MC(answer7.2.3, letters[1:3], '6e7a8c1c098e8817e3df3fd1b21149d1')
}
...
test_7.3 <- function() {
    check_DF(boot_SLR_CIs,
            c('statistic','avg','lower_ci','upper_ci'),
             "c01f179e4b57ab8bd9de309e6d576c48",
             c("avg", "lower_ci", "upper_ci"),
             c(100,100,100),
             c("31350c7b31147384bbfc080ccf60bc6c",
               "2d3c85558e8d44e2cf5b8188bc3a2951",
               "ceea588f5ab43789fd367a623049b33c"))
}
