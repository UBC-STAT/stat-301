...
test_2.9 <- function() {
  test_that('Did not assign answer to an object called "tiktok_bootstrap_results"', {
    expect_true(exists("tiktok_bootstrap_results"))
  })

  test_that("Solution should be a data frame", {
    expect_true("data.frame" %in% class(tiktok_bootstrap_results))
  })

  expected_colnames <- c("obs_test_stat", "pvalue")
  given_colnames <- colnames(tiktok_bootstrap_results)
  ...
  test_that("Data frame does not contain the correct number of rows", {
    expect_equal(digest(as.integer(nrow(tiktok_bootstrap_results))), "4b5630ee914e848e8d07221556b0a2fb")
  })

  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(tiktok_bootstrap_results$obs_test_stat) * 1e5)), "230d8ed07fee633f974205193056006b")
    expect_equal(digest(as.integer(sum(tiktok_bootstrap_results$pvalue) * 1e5)), "1473d70e5646a26de3c52aa1abd85b1f")
...
test_3.1 <- function() {
  ...
  test_that("answer3.1 should be a data frame", {
    expect_true("data.frame" %in% class(answer3.1))
  })
  
  test_that("answer3.1 does not contain the correct data", {
    expect_equal(digest(as.integer(sum(answer3.1$p_value) * 1e5)), "634bb5b4ea8305f2b5a4fb11f6c23555")
  })
...
test_3.2 <- function() {
  ...
  test_that('Did not assign answer to an object called "dwell_time_pvalue_evolution"', {
    expect_true(exists("dwell_time_pvalue_evolution"))
  })

  test_that("Solution should be a ggplot object", {
    expect_true(is.ggplot(dwell_time_pvalue_evolution))
  })

  properties <- c(dwell_time_pvalue_evolution$layers[[1]]$mapping, dwell_time_pvalue_evolution$mapping)
  ...
  test_that("Plot does not have the correct layers", {
    expect_true("GeomLine" %in% class(dwell_time_pvalue_evolution$layers[[1]]$geom))
  })

  test_that("x-axis label should be descriptive and human readable", {
    expect_false(dwell_time_pvalue_evolution$labels$x == toString(rlang::get_expr(properties$x)))
  })

  test_that("y-axis label should be descriptive and human readable", {
    expect_false(dwell_time_pvalue_evolution$labels$y == toString(rlang::get_expr(properties$y)))
  })

  test_that("Plot should have a title", {
    expect_true("title" %in% names(dwell_time_pvalue_evolution$labels))
  })
...
test_3.4 <- function() {
  ...
  test_that("Data frame does not contain the correct data", {
    expect_equal(digest(as.integer(sum(answer3.4$n_rejections) * 1e3)), "f5b46d85d364dcd91922018720a2c475")
  })
