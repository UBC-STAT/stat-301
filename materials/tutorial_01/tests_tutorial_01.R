...
# Helpers for Questions 4.5.1--4.5.3

check_manual_bootstrap_100 <- function(answerX.X) {
    var_name <- deparse(substitute(answerX.X))
    ...
    test_that(paste(var_name, 'should be a data frame'), {
        expect_true('data.frame' %in% class(answerX.X))
    })

    test_that(paste(var_name, 'does not have the correct columns'), {
        expect_equal(colnames(answerX.X), c('boot_intercept', 'boot_slope'))
    })

    test_that(paste(var_name, 'should contain B = 10000 bootstrap estimates'), {
        expect_equal(nrow(answerX.X), 10000)
    })

    test_that(paste(var_name, 'should contain numeric finite coefficient estimates'), {
        expect_true(is.numeric(answerX.X$boot_intercept))
        expect_true(is.numeric(answerX.X$boot_slope))
        expect_true(all(is.finite(answerX.X$boot_intercept)))
        expect_true(all(is.finite(answerX.X$boot_slope)))
    })

    # Recreate the fixed n = 100 sample and a small number of the bootstrap
    # replicates. This checks the same bootstrap construction as Question 4.4
    # without repeating all 10,000 regressions inside the test.
    ...
    set.seed(123)
    expected_sample100 <- caschools %>% slice_sample(n = 100)

    test_that('caschools_sample100 is not the required fixed sample of size 100', {
        expect_equal(caschools_sample100, expected_sample100)
    })

    expected_first <- replicate(20, {
        sample_n(expected_sample100, 100, replace = TRUE) %>%
            lm(formula = read ~ income, data = .) %>%
            .$coef
    })
    expected_first <- data.frame(
        boot_intercept = expected_first[1, ],
        boot_slope = expected_first[2, ]
    )

    test_that('The bootstrap estimates were not generated as in Question 4.4', {
        expect_equal(answerX.X[1:20, ], expected_first, tolerance = 1e-10)
    })

    print('Success!')
}

check_manual_bootstrap_plot_100 <- function(answerX.X) {
    var_name <- deparse(substitute(answerX.X))
    ...
    test_that(paste(var_name, 'should be a ggplot object'), {
        expect_true(is.ggplot(answerX.X))
    })
    ...
    test_that('Plot should be a histogram', {
        expect_true('GeomBar' %in% class(answerX.X$layers[[1]]$geom))
    })

    test_that('Plot should use all 10000 bootstrap estimates from lm_boot100', {
        expect_equal(nrow(answerX.X$data), 10000)
        expect_equal(answerX.X$data$boot_slope, lm_boot100$boot_slope)
    })
...
check_infer_sampling_plot_250 <- function(sample_data, sampling_plot) {
    sample_name <- deparse(substitute(sample_data))
    plot_name <- deparse(substitute(sampling_plot))

    test_that(paste(sample_name, 'should be a data frame with 250 observations'), {
        expect_true('data.frame' %in% class(sample_data))
        expect_equal(nrow(sample_data), 250)
        expect_equal(colnames(sample_data), colnames(caschools))
    })
    ...
    set.seed(123)
    expected_sample250 <- caschools %>% slice_sample(n = 250)

    test_that('caschools_sample250 is not the required fixed sample of size 250', {
        expect_equal(sample_data, expected_sample250)
    })

    test_that(paste(plot_name, 'should be a ggplot object'), {
        expect_true(is.ggplot(sampling_plot))
    })

    properties <- c(sampling_plot$layers[[1]]$mapping, sampling_plot$mapping)

    test_that(paste(plot_name, 'should have stat on the x-axis'), {
        expect_true('stat' == rlang::get_expr(properties$x))
    })

    test_that(paste(plot_name, 'should be a histogram'), {
        expect_true('GeomBar' %in% class(sampling_plot$layers[[1]]$geom))
    })

    test_that(paste(plot_name, 'should contain B = 10000 bootstrap slope estimates'), {
        expect_equal(nrow(sampling_plot$data), 10000)
        expect_true('stat' %in% colnames(sampling_plot$data))
        expect_true(all(is.finite(sampling_plot$data$stat)))
    })

    test_that(paste(plot_name, 'should contain a non-degenerate bootstrap distribution'), {
        expect_gt(sd(sampling_plot$data$stat), 0)
    })

    test_that('The bootstrap distribution should be centered near the fitted sample slope', {
        sample_slope <- unname(coef(lm(read ~ income, data = sample_data))[2])
        expect_lt(abs(mean(sampling_plot$data$stat) - sample_slope), 0.05)
    })

    test_that(paste(plot_name, 'x-axis label should be descriptive and human readable'), {
        expect_false(sampling_plot$labels$x == toString(rlang::get_expr(properties$x)))
    })

    test_that(paste(plot_name, 'should have a title'), {
        expect_true('title' %in% names(sampling_plot$labels))
    })
...
test_2.1 <- function() {
  test_that('Did not assign answer to an object called "caschools_dim"', {
    expect_true(exists("caschools_dim"))
  })
  
  answer_as_numeric <- as.numeric(caschools_dim)
  test_that("Solution should be a number", {
    expect_false(is.na(answer_as_numeric[1]))
    expect_false(is.na(answer_as_numeric[2]))
  })
  
  test_that("Solution should be an integer", {
    expect_true(answer_as_numeric[1] %% 1 == 0)
    expect_true(answer_as_numeric[2] %% 1 == 0)
  })
  
  test_that("Solution is incorrect", {
    expect_equal(digest(as.integer(answer_as_numeric[1])), "d6af036ffbdd7ccfc34dc7862b0e50d3")
    expect_equal(digest(as.integer(answer_as_numeric[2])), "234a2a5581872457b9fe1187d1616b13")
...
# Question 3.1
test_3.1.0 <- function() {
    check_MC(answer3.1_response, c("read","income"),
             '5aef5bcbb5ac582272cbbb8e75b50902')
}

test_3.1.1 <- function() {
    check_MC(answer3.1_predictor, c("read","income"), '86ceffa2bf0fe2928a32579194f25acc')
}
...
test_4.4 <- function() {
    check_DF(lm_boot, 
            c("boot_intercept", "boot_slope"),    
             "f1a30baa3072c1aad822c059f35c6841",
             c("boot_intercept", "boot_slope"),
             c(100, 10000),
             c("20b44617e558a7d1a1fbf45e4377739e",
               "d5983dd0e9b5a1810f8ce9dbbd2822e6"))
...
test_4.5 <- function() {
    check_plot(slope_sampling_dist_boot, "boot_slope",
               "GeomBar", F, "3e2e4a08c44d0224de5b7e668c75ace3", "f1a30baa3072c1aad822c059f35c6841",
               "1496f567c87438539878d7e7e94f560e", T)
}
...
test_4.7.1 <- function() {
    check_manual_bootstrap_100(lm_boot100)
}
...
test_4.7.2 <- function() {
    check_manual_bootstrap_plot_100(slope_sampling_dist_100)
}
...
test_4.7.3 <- function() {
    check_infer_sampling_plot_250(caschools_sample250, slope_sampling_dist_250)
}
...
test_4.8 <- function() {
    check_MC(answer4.8, LETTERS[1:3], '6e7a8c1c098e8817e3df3fd1b21149d1')
}
...
test_4.9 <- function() {
    check_plot(slope_sampling_dist_boot_limits, "boot_slope",
               "GeomBar", T, "3e2e4a08c44d0224de5b7e668c75ace3", "f1a30baa3072c1aad822c059f35c6841",
               "1496f567c87438539878d7e7e94f560e", T)
}
